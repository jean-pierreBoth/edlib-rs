...
pub struct EdlibAlignConfigRs<'a> {
    ...
    additionalEqualitiesRs : &'a[EdlibEqualityPairRs],
}


impl EdlibAlignConfigRs {

    /// Helper method for easy construction of configuration object.
    /// return Configuration object filled with given parameters.
    pub fn new(k : u32, mode : EdlibAlignModeRs, 
                                task : EdlibAlignTaskRs,
                                additionalEqualities :  &'a[EdlibEqualityPairRs]) -> EdlibAlignConfigRs {
...
impl Default for EdlibAlignConfigRs {
    ///      k = -1, mode = EDLIB_MODE_NW, task = EDLIB_TASK_DISTANCE, no additional equalities.
    fn default() -> Self {
        EdlibAlignConfigRs{k:-1, task : EDLIB_TASK_DISTANCE, additionalEqualities : &[]}
    }
}
...
struct EdlibAlignResultRs {
    ...
    /// Set to NULL if edit distance is larger than k.
        // From C: If you do not free whole result object using edlibFreeAlignResult(), do not forget to use free().
    endLocations : Option<Vec<u32>>,
